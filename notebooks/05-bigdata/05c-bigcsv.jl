...
using Parsers, Dates, DataFrames, CSV, OnlineStats
...
highlight(md"""
- In the real world, it is entirely possible to run into a CSV file that is larger than your laptop's memory.
- One solution would be to rent compute on a cloud computing platform.
""")
...
dataset = CSV.write(tempname(), DataFrame(x=cumsum(randn(10^7))))

# ╔═╡ 7ae9df5d-3948-457a-be0d-88d03bc23366
md"## The Load-it-all Approach"

# ╔═╡ b9b1a35d-c169-48d1-84bb-780384d35222
function analysis1(path)
	df = CSV.read(path, DataFrame)
	sum(df.x)
end

# ╔═╡ 9cf6eaaf-6f16-4ded-8d0a-71e5fd6d4f50
@time analysis1(dataset)
