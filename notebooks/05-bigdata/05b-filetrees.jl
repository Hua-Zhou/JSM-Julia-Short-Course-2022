...
# ╔═╡ cb4d09aa-3ef7-4f44-88d5-53e3efea1225
using FileTrees, CSV, DataFrames
...
# ╔═╡ a16d5d26-344f-45a7-bad9-ae15b416c472
md"# FileTrees.jl"

# ╔═╡ 80149372-fce1-4da6-8be0-de5c99cbc266
highlight(md"""
#### Think mapreduce for files.

- *map* a function to each file (e.g. read as DataFrame and perform a calculation)
- *reduce* function outputs into a single result.
""")

# ╔═╡ c25fa0e8-6c9e-42a3-ac81-f6bab60dd394
highlight(md"""
#### Powered by **Dagger.jl**

- **Dagger** is a distributed computing scheduler.
- You can use `addprocs()` to add Julia workers (doesn't work inside Pluto 🙁).
""")

# ╔═╡ 71dbb91a-0b90-4fbe-a10b-888ac12d4ff8
md"""
## Example: Stock Market Data

(Kaggle's ["Huge Stock Market Dataset"](https://www.kaggle.com/datasets/borismarjanovic/price-volume-data-for-all-us-stocks-etfs))

- Over 7000 CSVs (one for each stock) of histogrical OHLC data.
"""

# ╔═╡ ec9270b3-6f35-40af-95e6-7c108d1c3167
stocks = "/Users/joshday/datasets/stockdata/Stocks/"

# ╔═╡ 6b7fc75b-e015-4402-9dd1-4df45c654c15
tree = FileTree(stocks);

# ╔═╡ 6575d762-09ed-4bc2-96ae-689d03f63d6b
highlight(md"#### You can \"query\" files by name:")

# ╔═╡ 95e2105c-8fb7-43d4-b1bc-32adc6142cfb
tree[glob"aap*"]

# ╔═╡ 6295bfe5-73a3-4698-b4e5-6585b72fb242
subtree = tree[r"aapl|msft|nflx|ibm[^a-z]"]

# ╔═╡ 9cbd9dfa-6f34-4096-aca2-4c56282dede9
md"## Lazy Operations on FileTrees"

# ╔═╡ 581fabe5-6287-4c84-9390-6f816683e8ff
md"""

##### Be lazy in our calculations? $(@bind be_lazy CheckBox())
"""

# ╔═╡ c8bbbb71-ccf6-474c-b46c-759180611c7c
subtree_dfs = FileTrees.load(subtree, lazy=be_lazy) do file
	temp = DataFrame(CSV.File(joinpath(stocks, name(file))))
	temp[:, :file] .= name(file)
	temp
end
...
nprocs()  # 1 unless you start julia with `julia -p n`

addprocs()  # Add `Sys.CPU_THREADS` workers

nprocs()  # Now: `1 + Sys.CPU_THREADS`
