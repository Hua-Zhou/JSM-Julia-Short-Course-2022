...
# ╔═╡ b670bab8-f04a-4510-846d-c7e4ba75c8a1
...
# ╔═╡ 4b73c71d-1faa-475a-a8e2-053449efff7f
md"# Weighting"

# ╔═╡ 20387285-b17b-432e-bb37-e3f879ab9178
highlight(md"""
- In long-running calculations, you may want more recent observations to have more influence over the statistic.

- **OnlineStats** offers a novel weighting system for controlling this.
""")

# ╔═╡ 90f43765-76d9-4b79-a1ab-f14a4fc6e987
md"""
Let's revisit the online update for mean:
...
We can generalize the *influence* of the newest observation with:


```math
\bar x_n = \left(1-w_n \right) x_n + w_n \bar x_{n-1}
```
"""

# ╔═╡ c7c388bc-d4ce-4cfc-873d-7a93e50f759c
highlight(md"""
### Weighting should be a major concern when dealing with *concept drift*.
""")

# ╔═╡ 9bae271f-889d-4a0d-90df-757719b0d1d2
md"![](https://user-images.githubusercontent.com/8075494/57347308-d4d27d00-711f-11e9-8fbe-fc4523b96b48.gif)"

# ╔═╡ 85225868-3709-4131-891f-d549c39b12a3
highlight(md"""
### Weighting can also be used for smoothing
""")

# ╔═╡ 4d8c1d60-4b28-43f8-8a9c-2525c8bf690c
y = cumsum(randn(500));

# ╔═╡ 648c018d-acd6-4e79-b902-dba420932959
let 
	m1 = Mean(weight = EqualWeight())
	m2 = Mean(weight = ExponentialWeight(.1))
	
	res1 = [value(fit!(m1, yi)) for yi in y]
	res2 = [value(fit!(m2, yi)) for yi in y]
	plot([y res1 res2], lab=["Data" "EqualWeight" "ExponentialWeight(.1)"], lw=[1 2 2])
end

# ╔═╡ 82ab7a6e-a9ad-4a52-b24e-2d2f77ac209c
md"# Data Visualization"

# ╔═╡ ecfc9b8b-5052-4a50-be58-ee894f825a6e
highlight(md"""
### **OnlineStats** has several types for visualizing *infinitely*-large datasets.
""")

# ╔═╡ effd4410-05ed-4bc0-8975-bfd51dbe0147
highlight(md"""
### The Idea:

- Like a histogram, but instead of the count of observations, you calculate a statistic of the observations that fall inside each bin.
- When we run out of bins, we start merging bins together.

---
- The downside is that you only get a *coarse* view of your data.
- The upside is that your data is plot-able!  No matter how big it is!
""")

# ╔═╡ 5e43f574-fb3e-4899-a711-cda36783f8a5
md"### Univariate Data"

# ╔═╡ b27562dd-a29b-4193-aea3-3e03dd8d841e
let 
	o = Partition(Extrema(), 200)
	y = randn()
...
# ╔═╡ d78a5243-ef52-40a3-8782-694c6d783719
md"""
### Bivariate Data

- Calculating this in a way where you don't need to specify the extrema of either variable is deceptively hard!
"""

# ╔═╡ b8dc4159-b702-4c0c-b40a-cd7305dbbd16
let 
	o = IndexedPartition(Float64, KHist(15))
	for i in 1:1_000_000
		x = randn()
		y = x + randn()
		fit!(o, x => y)
...
# ╔═╡ 8de981ad-964b-4088-a8f1-cd183ec33e2f
md"# Big Data the Easy Way"

# ╔═╡ d3690ccf-8a78-4b7e-87e4-77a6ac8db6b7
highlight(md"### (but not the fastest way)")

# ╔═╡ 2885f289-0e0f-43ad-b9fb-86ead5239ca1
md"""
```julia
# Step 1) Split data into manageable chunks
# e.g. multiple SQL queries, split up CSV file, etc.


# Step 2) Figure out what you want to calculate.
stat = ExpandingHist(400)

# Step 3) Load each chunk and update your `stat`
for chunk in chunks
	data = load(chunk)
	fit!(stat, data)
end

# Step 4) Look at your stat
plot(stat)
```
"""
...
begin
	...
	serialize(joinpath(path, "chunk1"), randn(10_000))
	serialize(joinpath(path, "chunk2"), randn(10_000))
	serialize(joinpath(path, "chunk3"), randn(10_000))
	serialize(joinpath(path, "chunk4"), randn(10_000))
end;

# ╔═╡ c633679f-8b82-4479-abe5-25b3acb1d927
begin 
	stat = ExpandingHist(100)

	# Load data chunk by chunk and update `stat`
	for i in 1:4
		data = deserialize(joinpath(path, "chunk$i"))
		fit!(stat, data)
	end
